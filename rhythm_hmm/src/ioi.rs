...
impl Ioi3Prob {
    ...
    pub fn observe(&self, hidden: &Ioi3, obs: &Ioi3) -> f32 {
        ...
        let (tempo_mean, tempo_dev) = self.mean_and_dev(tempo.as_slice());
        let tempo_prob = if tempo_dev / tempo_mean < self.tempo_tolerance {
            1.0
        } else {
            self.tempo_tolerance / (tempo_dev / tempo_mean)
        };
        // println!("obs {:?} from {:?}: {:?}",
        //          hidden.iois,
        //          obs.iois,
    ...
    pub fn transition(&self, obs: &Ioi3, next: u32) -> f32 {
        if obs.iois.iter().all(|&ioi| ioi == next) {
            0.7
        } else if obs.iois.iter().all(|&ioi| next % ioi == 0 || ioi % next == 0) {
            0.5
        } else {
            0.1
        }
